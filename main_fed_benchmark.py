...
# -*- coding: utf-8 -*-
"""
Main script for federated learning with FBL benchmark wireless model.

This script implements federated learning with the benchmark FBL wireless model
from wireless_FBL_benchmark.py instead of the standard wireless model.
"""
...
if __name__ == '__main__':
    ...
    for iter in range(args.round):
        ...
        if args.scenario == 'woPER':  # Without packet error option
            ...
            # Use benchmark user selection instead of standard user selection
            idxs_users, proba_success_avg, fails, errorprob, obj_values = user_selection_benchmark(
                args, wireless_arg, seed_round, datasize_weight, weights, later_weights
            )
