"""
wireless_FBL_benchmark.py - Wireless communication functions using FBL error model

This script implements wireless communication functions for federated learning
similar to wireless.py, but uses the finite blocklength (FBL) error probability model
...
import copy
import pickle 
from scipy.stats import norm  # For FBL error calculation
...
def wireless_param(args, data_weight, nb_data_assigned):
    ...
    wireless_arg = { 
        ...
        'M': 16,
        'Mprime': 15,
        'E_max': args.E_max if hasattr(args, 'E_max') else 60,  # mJ
        'Tslot': 1.3,
        'sigma': args.sigma,  # 1
        'freq': 2400,  # MHz
        'P_max': 10,  # mW
        ...
        'freq_comp': 2*10**9,
        'C': 2*10**4,
        'Packet_size': args.packet_size if hasattr(args, 'packet_size') else 500,
        'T_total': args.total_blocklength if hasattr(args, 'total_blocklength') else 1000
    }
    ...
    # Calculate blocklength per user
    wireless_arg['blocklength'] = wireless_arg['T_total'] // args.active_UE if wireless_arg['T_total'] > 0 else 500
    
    # Calculate SNR and success probability using FBL model
    ...
    rate = wireless_arg['Packet_size'] / wireless_arg['blocklength']
    
    # Calculate error probability using FBL model
    err_prob = error_prob_fbl(snr, wireless_arg['blocklength'], rate)
    
    # Success probability is complement of error probability
    wireless_arg['success prob'] = 1 - err_prob
    ...
    # Calculate weights for Salehi method if needed
...
def update_wireless(args, wireless_arg, seed):
    """
    Update wireless channel conditions for the current round.
    
    Args:
    args: argparse.Namespace object containing program configuration
    wireless_arg: dict of wireless parameters
    seed: int, random seed for the current round
    
    Returns:
    dict: Updated wireless parameters
    """
...
def objective_function(x, alpha, h_i, S_i, data_size, const, later_weights, P_max, incr, decr):
...
def user_selection(args, wireless_arg, seed, data_size, weights, later_weights):
    ...
    wireless_arg: dict of wireless parameters
    seed: int, random seed for the current round
    data_size: numpy array of data sizes for each client
    weights: numpy array of weights for each client
    later_weights: numpy array of additional weights for each client
    
    Returns:
    list: Indices of successfully transmitting clients
    numpy.ndarray: Average success probability for each client
    int: Number of transmission failures
    float: Average success rate of selected clients
    ...
    blocklength = wireless_arg['blocklength']
    packet_size = wireless_arg['Packet_size']
    
    K = args.active_UE
    
    # Standard user selection strategies
    if (args.selection == 'uni_random' or 
        args.selection == 'best_channel' or 
        args.selection == 'best_channel_ratio' or 
        args.selection == 'best_loss' or 
        args.selection == 'weighted_random' or
        args.selection == 'salehi' or 
        (wireless_arg['decr'] == 0 and (not args.no_later or not args.selection == 'solve_opti_laterW'))):
        
        # Various user selection methods
        ...
        # Extract parameters for selected clients
        ...
        run_time = 0
        message = ''
        obj_value = objective_function(power_allocated, const_alpha, h_avg, np.ones(args.total_UE), data_size, theta, later_weights, P_max, 1, 1)
    
    # Advanced optimization-based user selection strategies
    elif (args.selection == 'solve_opti_loss_size' or 
          args.selection == 'solve_opti_loss_size2' or 
          args.selection == 'solve_opti_loss_size3' or 
          args.selection == 'solve_opti_loss_size4' or 
          args.selection == 'solve_opti_size' or 
          args.selection == 'solve_opti_loss' or 
          args.selection == 'solve_opti_AoU' or 
          args.selection == 'solve_opti_laterW'):
        
        if args.opti == 'P4':
            user_selected, power_allocated, message, _, run_time = user_selection_opti(
                args.active_UE, wireless_arg['M'], wireless_arg['Mprime'],
            ...
            message = 'yoyo'
            active_clients, power_active, run_time = solve_opti_all(
                all_comb, list2choose, weights, h_avg, N0, B, m, K,
                wireless_arg['alpha'], wireless_arg['beta'],
            ...
            if args.test_method == 'sum_loss':
                _, power_allocated, _, _ = opti_LR(K, const_alpha, h_avg, np.ones(args.total_UE) * wireless_arg['decr'], P_max, P_sum, data_size, const, later_weights + weights_to_use)
            else:
                if args.no_later:
                    _, power_allocated, _, _ = opti_LR(K, const_alpha, h_avg, weights_to_use, P_max, P_sum, data_size, const, np.zeros(args.total_UE))
                else:
                    _, power_allocated, _, _ = opti_LR(K, const_alpha, h_avg, weights_to_use * wireless_arg['decr'], P_max, P_sum, data_size, const, later_weights)
        ...
        obj_value = objective_function(power_allocated, const_alpha, h_avg, weights, data_size, theta, later_weights, P_max, 1, 1)
    ...
    # Communication using FBL error model
    ...
    mask_avg = snr_avg > 0
    if np.any(mask_avg):
        rate_avg = packet_size / blocklength
        err_prob_avg = error_prob_fbl(snr_avg[mask_avg], blocklength, rate_avg)
