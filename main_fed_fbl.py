...
# -*- coding: utf-8 -*-
"""
Federated Learning with Finite Blocklength (FBL) Communications

This script implements federated learning with a focus on realistic wireless communications
using the Finite Blocklength (FBL) model. It extends main_fed.py by replacing the user
selection mechanism with FBL-aware selection from selections/wireless_FBL.py.
"""
...
from selections.wireless_FBL import user_selection_fbl  # Import the FBL-based user selection
    ...
    for iter in range(args.round):
        ...
        if args.scenario == 'PER':
            if len(idxs_users) != 0:
                ...
                # Log to whatever monitoring system you use
            else:
                log_dict = {
                    "loss": training_loss_avg, 
                    "accuracy": acc_test, 
